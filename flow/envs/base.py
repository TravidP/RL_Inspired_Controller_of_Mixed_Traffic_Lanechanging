    ...
    * compute_reward
    ...
    def setup_initial_state(self):
        ...
        if self.initial_config.shuffle:
           ...
           # ===== Custom even-spacing shuffle for RL vehicles =====
            if self.initial_config.shuffle:
                random.shuffle(self.initial_ids)  # 先打乱

                # 获取所有 RL 车辆的 ID
                rl_ids = [vid for vid in self.initial_ids if vid.startswith("rl")]
                human_ids = [vid for vid in self.initial_ids if not vid.startswith("rl")]
                ...
                if len(rl_ids) >= 2:
                    ...
                    even_index = random.randrange(0, num_humans, 2)  # 偶数间隔位置

                    # 构建新的排列
                    ordered_ids = []
                    ordered_ids.append(rl0)
                    ordered_ids.extend(human_ids[:even_index])
                    ordered_ids.append(rl1)
                    ordered_ids.extend(human_ids[even_index:])

                    self.initial_ids = ordered_ids

                    # print(f"[Init setup] RL0 at index 0, RL1 after {even_index} humans (even spacing).")
                else:
                    print("[Init setup] Less than two RL vehicles, skipping custom reorder.")
